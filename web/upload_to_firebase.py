...
def initialize_firebase():
    # 절대 경로 설정
    base_path = "C:\\Users\\user\\Desktop\\Firebase"
    key_path = os.path.join(base_path, 'firebase-key.json')

    # firebase-key.json 파일 확인
    if not os.path.exists(key_path):
        raise FileNotFoundError(f"'firebase-key.json' 파일이 경로에 없습니다: {key_path}")
...
def upload_data_to_firestore():
    ...
    if not os.path.exists(data_path):
        raise FileNotFoundError(f"'sensor_data.json' 파일이 경로에 없습니다: {data_path}")
