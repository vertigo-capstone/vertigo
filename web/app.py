...
cred = credentials.Certificate(
    "./firebase/vertigo-firebase.json")  # JSON 파일 경로
...
def get_sensor_data():
    ...
    sensor_data = {}
    try:
        gyroscope_doc = db.collection('vertigo').document('vertigo').get()
...
def do_login():
    ...
    if username == '1234' and password == '1234':  # 실제 아이디와 비밀번호로 수정
        ...
        # 로그인 실패 메시지
        return render_template('login.html', message="로그인에 실패하였습니다.")
...
def dashboard():
    ...
    # 대시보드에 데이터 출력
    return render_template('dashboard.html', username=username, sensor_data=sensor_data)
